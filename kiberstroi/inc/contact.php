...
if (empty($action)) {
	...
	<div class='contact-content'>
		<h1 class='contact-title'>Вызов замерщика:</h1>
		<div class='contact-loading' style='display:none'></div>
		<div class='contact-message' style='display:none'></div>
		<form action='#' style='display:none'>
			<label for='contact-name'>*Ваше имя:</label>
			<input type='text' id='contact-name' class='contact-input' name='name' tabindex='1001' />
			<label for='contact-message'>*Ваш телефон:</label>
			<input type='text' id='contact-message' class='contact-input' name='message' tabindex='1002' />
			<label for='contact-comment'>Дополнительные комментарии:</label>
	...
	if ($token === smcf_token($to)) {
		...
		echo "Ваш заявка успешно отправлена.";
	}
	else {
...
function smcf_send($name, $email, $subject, $message, $cc) {
	...
	$body = "Новая заявка на вызов замерщика с сайта.\n\nОтправитель: $name\n\n";
	...
	$subject = "Новая заявка на вызов замерщика с сайта";
	...
	@mail($to, $subject, $body, $headers) or 
		die("Ошибка отправки письма сервером.");
}
